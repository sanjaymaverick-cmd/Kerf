...
/**
 * Persist PGLite to disk when KERF_DATA_DIR is set (USB stick / shop PC).
 * Vercel and the live preview leave this unset — in-memory, wiped on restart.
 */
function pgliteDataDir(): string | undefined {
  const explicit = typeof process !== "undefined" ? process.env.KERF_DATA_DIR?.trim() : undefined;
  if (explicit) return explicit;
  if (typeof process !== "undefined" && process.env.KERF_STICK === "1") {
    return "data/pglite";
  }
  return undefined;
}
...
async function createPgliteSql(): Promise<Sql> {
  ...
  globalRef.__pgliteInstance__ ??= (async () => {
    ...
    if (dataDir) console.log(`[db] PGLite on disk at ${dataDir}`);
    await pg.waitReady;
    await pg.exec(
